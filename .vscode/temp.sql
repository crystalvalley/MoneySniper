...
CREATE TABLE IF NOT EXISTS moneysniper.members
(
...
CREATE TABLE IF NOT EXISTS moneysniper.roles
(
    `role_id`      INT                 NOT NULL    AUTO_INCREMENT COMMENT '권한 ID', 
    `post_role`    INT                 NULL        COMMENT '게시글(On, Off)', 
    `member_role`  INT                 NULL        COMMENT '회원(On, Off)', 
    PRIMARY KEY (role_id)
);
...
CREATE TABLE IF NOT EXISTS moneysniper.member_datas
(
    `email`    INT            NOT NULL    AUTO_INCREMENT COMMENT '이메일', 
...
CREATE TABLE IF NOT EXISTS moneysniper.stocks
(
...
CREATE TABLE IF NOT EXISTS moneysniper.posts
(
...
CREATE TABLE IF NOT EXISTS moneysniper.comments
(
    `comment_id`   INT            NOT NULL    AUTO_INCREMENT COMMENT '댓글 ID', 
    `post_id`      VARCHAR(45)    NULL        COMMENT '게시글 ID', 
...
CREATE TABLE IF NOT EXISTS moneysniper.transactions
(
    ...
    `stock_code`      VARCHAR(45)              NULL        COMMENT '종목코드', 
...
CREATE TABLE IF NOT EXISTS moneysniper.stockholdings
(
    `owner`       INT            NOT NULL    AUTO_INCREMENT COMMENT '소유자', 
    `stock_code`  VARCHAR(45)    NULL        COMMENT '종목 코드', 
